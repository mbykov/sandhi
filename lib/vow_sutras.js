...
var sutras = [
    ...
    {num: '6.1.101',
     ...
     only: 'ext',
     split: function(mark) {
         // log('similar', mark); // здесь пока что фильтр полностью определяется в маркере - dirgha_liga
         var dirgha = mark.pattern;
         var hrasva = u.hrasva(dirgha); // full-form
         var sandhi_d = [dirgha, ' ', hrasva].join('');
         var sandhi_h = [u.liga(hrasva), ' ', hrasva].join('');
         return [sandhi_d, sandhi_h];
     },
     ...
     add: function(test) {
         ...
         if (u.c(Const.aAliga, test.fin) && u.c(Const.aA, test.beg)) {
             ...
             // log('similar', test)
             test.first.pop();
             test.first.push(u.liga(u.dirgha(test.fin)));
         } else {
             return;
         }
         res.push(test);
