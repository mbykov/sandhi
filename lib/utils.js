...
utils.prototype.liga = function(v) {
    // nodejs sorts its incorrect !! Use Const.vow2liga instead
    // return Const.allligas[Const.allvowels.indexOf(v)];
}

utils.prototype.jhal2jaS = function(c) {
    if (isIN(['ग', 'ध', 'ख', 'क', 'ह'])) return 'ग' // g
    else if (isIN(['ग', 'ध', 'ख', 'क', 'ह'])) return 'ग' // j
    else if (isIN(['ड', 'ढ', 'ठ', 'ट', 'ष'])) return 'ड' // D
    else if (isIN(['द', 'ध', 'थ', 'त', 'स'])) return 'ड' // d
    else if (isIN(['ब', 'भ', 'फ', 'प'])) return 'ब'; // b
}
