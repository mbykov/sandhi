...
utils.prototype.combinator = function(arr, samasa) {
    var fn = function(active, rest, a) {
        ...
        // only three segments:
        if (active.length > 2) return; // the same as only combination.length < 3
        ...
        // var sects = active.map(function(m) {  return _.range(m.pos, m.pos+m.pattern.length + 2)});
        var sects = active.map(function(m) {  return _.range(m.pos, m.pos+m.pattern.length)});
        var flat = _.flatten(sects);
        // log('sects', sects, 'flat', flat, 'uniq:', _.uniq(flat));
        // if (flat.length != _.uniq(flat).length) log('not uniq', flat);
