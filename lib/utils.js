...
// vowel from semi-vowel => .base()
// utils.prototype.semi2vow = function(sym) {
//     return vowrow(sym)[1];
// }
...
utils.prototype.isConst = function(sym) {
...
utils.prototype.combine = function(f, s, ends, starts) {
    // FIXME: щедро я клонов навтыкал - аж два раза
    first = _.clone(f);
    second = _.clone(s);
    first.pop(); // FIXME: это потому, что я не убрал pattern из first - убрать
    second.shift();
    var res = {};
    var tmp;
...
utils.prototype.combinator = function(arr, samasa) {
    var fn = function(active, rest, a) {
        ...
        // only three segments:
        if (active.length > 2) return; // the same as only combination.length < 3, // 3 markers = 4 segments

        // only one marker on pos:
        ...
        //FIXME: тут не так просто +2, тут нужно отследить длинну слога - чтобы была гласная и вообще все ограничения, а не +2
        // FIXME: и +2 и flat отбрасывает нужные значения на правой границе слова
        // var sects = active.map(function(m) {  return _.range(m.pos, ((m.pos+m.pattern.length+4 > size) ? size : m.pos+m.pattern.length+4))});

        // regions should not intersect in any combination of sandhies:
        // +2 - якобы проверка на слог между маркерами - пока тесты на 2 слова - не влияет
        // var sects = active.map(function(m) {  return _.range(m.pos, m.pos+m.pattern.length + 1)});
        var sects = active.map(function(m) {  return _.range(m.pos, m.pos+m.pattern.length)});
        var flat = _.flatten(sects);
        if (flat.length != _.uniq(flat).length) return;

        // здесь образуются regions без гласных, но здесь их проверять накладно - или потом, в combinations?
        // похоже, здесь секции проверять нельзя, ибо они расширяются-наращиваются ? (тесты не проходят). Значит, только в main
        // if (!correct(sects, samasa)) return;
        // log(flat);
        ...
        if (active.length==0 && rest.length==0)
            return;
        if (rest.length==0) {
            // сохраняю в A только те комбинации, маркеры которых на расстоянии больше 2 (имитация поиска гласной в сегменте)
            // выглядит глупо, но выигрыш - 10-20%
            // FIXME: завтра верхнее попробовать перенести сюда?
            var asects = active.map(function(m) {  return m.pos });
            if (asects.length > 1) {
                var f = asects[0];
                var s = asects[1];
                if (s - f < 2) return;
                // log(samasa.length, samasa.length/7)
                // if (s - f < samasa.length/7) return;
            }
            a.push(active);
        } else {
