...
var sutras = {
    ...
    '6.1.77': {
        ...
        del: function(mark) {
            ...
            mark.first.pop();
            var starts;
            var ends = [u.liga(u.base(mark.pattern)), u.dliga(u.base(mark.pattern))];
    ...
    // diphthong followed by any vowel, including itself, changes to its semi-vowel equivalent - ext - optl
    '6.1.78': {
        type: 'vowel',
        only: 'ext',
        split: function(mark) {
        ...
        del: function(mark) {
            // log('DEL 78', mark);
            if (mark.fin == Const.A) mark.first.pop();
            var ends = [u.gliga(u.base(mark.pattern)), u.vliga(u.base(mark.pattern))];
            var starts = [mark.beg, u.dirgha(mark.beg)];
            // log('S', u.base(mark.pattern), u.vowel(u.vliga(u.base(mark.pattern)))); //
            return u.combine(mark.first, mark.second, ends, starts);
        },
