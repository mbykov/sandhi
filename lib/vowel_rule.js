...
rules.savarnadirgha = {
    ...
    method: function(test) {
        // log('SIM', test.fin, test.beg, u.similar(test.fin, test.beg))
        if (!u.similar(test.fin, test.beg)) return;
        if (!u.c(Const.consonants, test.fin)) test.first.pop();
        test.second.shift();
        log('=====METHOD 101====');
        return test.first.concat(Const.dirghaLiga[test.beg], test.second).join('');
    }
}
...
rules.s6_1_88 = {
    ...
    vowel: true,
    method: function(test) {
        if (!u.aA(test.fin) || !u.c(Const.diphtongs, test.beg)) return;
        if (!u.c(Const.consonants, test.fin)) test.first.pop();
        test.second.shift();
        log('=====METHOD 88====');
        return test.first.concat(Const.vriddhiLiga[test.beg], test.second).join('');
    }
}
...
rules.s6_1_87 = {
    ...
    vowel: true,
    method: function(test) {
        if (!u.aA(test.fin) || !u.c(Const.iurlIUR, test.beg)) return;
        if (!u.c(Const.consonants, test.fin)) test.first.pop();
        test.second.shift();
        log('=====METHOD 87====');
        return test.first.concat(Const.gunaLiga[test.beg], test.second).join('');
    }
}
...
rules.s6_1_78 = {
    ...
    method: function(test) {
        ...
        if (!u.c(Const.diphtongs, test.matra) || !u.c(Const.allvowels, test.beg)) return;
        var idx = Const.diphtongs.indexOf(test.matra);
        var semivow = ['य', 'व', 'ाय', 'ाव'][idx];
        test.first.pop();
        test.second.shift();
        log('=====METHOD 78====');
        return test.first.concat(semivow, Const.vow2liga[test.beg], test.second).join('');
    }
}

// the same, but external / optional
rules.s6_1_78_ext = {
    id: 140, // FIXME: все-таки массив?
    sutra: '6.1.78',
    vowel: true,
    method: function(test) {
        if (test.only == 'int') return;
        if (u.c(Const.gudiphs, test.matra) && Const.a == test.beg) return; // 6.1.109
        // log('=====METHOD 78====', u.c(Const.diphtongs, test.matra), u.c(Const.allvowels, test.beg));
        if (!u.c(Const.diphtongs, test.matra) || !u.c(Const.allvowels, test.beg)) return;
        log('=====METHOD 78 ext ====');
        var idx = Const.diphtongs.indexOf(test.matra);
        ...
        // log('T1', test.first, 'T2', test.second, 'IDX', idx, semivow, test.matra);
        var res1 = [test.first.join(''), test.second.join('')].join(' ');
        test.second.shift();
        var res2 = test.first.concat(semivow, Const.vow2liga[test.beg], test.second).join('');
        // log('R1', res1, 'R2', res2);
        return [res1, res2];
    }
}
...
rules.s6_1_109 = {
    ...
    method: function(test) {
        ...
        if (!u.c(Const.gudiphs, test.matra) || !(Const.a == test.beg)) return;
        test.second.shift();
...
rules.s6_1_77 = {
    ...
    vowel: true,
    method: function(test) {
        // log('==', test.matra, test.beg, Const.allsimples)
        if (u.c(Const.aA, test.matra)) return;
        if (u.similar(test.fin, test.beg)) return;
        if (!u.c(Const.allsimples, test.matra) || !u.c(Const.allvowels, test.beg)) return;
...
/*
  ...
  If x is Column1, Column2, Column3, Column4, sibilant or aspirate, then x is replaced by the corresponding Column3.
  ...
  == нет условия на second = кажется, случай, когда результат должен переписываться след. правилами ==
  A hard consonant followed by a soft consonant or vowel changes to the third of its class == это ли?
  4th. 3rd. 2nd. & 1st. letters of class consonants, the sibilant and the aspirate
*/
