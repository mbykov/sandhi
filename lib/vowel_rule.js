...
rules.s6_1_77 = {
    ...
    vowel: true,
    method: function(test) {
        if (u.c(Const.aA, u.matra(test.ends))) return;
        if (u.similar(test.ends, test.starts)) return;
        if (!u.c(Const.allvowels, u.matra(test.ends)) || !u.c(Const.allvowels, test.starts)) return;
        test.first.pop();
        test.second.shift(); // ================ тут а убрать, но не а - заменить
        var semi = Const.vow2semi[u.matra(test.ends)];
        log('=====METHOD 77 ====');
        return test.first.concat(Const.virama, semi, test.second).join('');
    }
}
